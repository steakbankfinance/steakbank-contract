...
contract LBNBStaking is Ownable {
    ...
    function deposit(uint256 _amount) public payable {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                skbToken.safeTransfer(address(msg.sender), pending);
            }
        }
        if(_amount > 0) {
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyOwner {
        require(_amount < skbToken.balanceOf(address(this)), 'not enough token');
        skbToken.safeTransfer(address(msg.sender), _amount);
    }
